...
const ProgressionManager = (function() {
    ...
    // 🌟 缓存的 auth UID（AuthManager 登录/登出时更新）
    ...
    function getEffectiveId() {
        return _cachedAuthUid || getPlayerId();
    }
    ...
    return {
        ...
        async onAuthLogin(uid, email){
            _cachedAuthUid = uid; data.email = email;
            // 🌟 登录后立即从云端拉取数据（用 auth.uid）
            const row = await loadFromCloud();
            if (row) { loadFromRow(row); saveLocal(); }
            saveToCloudNow(); this.refreshUI();
        },
        onAuthLogout(){
            _cachedAuthUid = null;
            // 登出后加载本地匿名数据
...
const AuthManager = {
    ...
    init(){
        ...
        sup.auth.onAuthStateChange((event,session)=>{
            if(event==='SIGNED_IN'&&session)this._onSignedIn(session);
            else if(event==='SIGNED_OUT')this._onSignedOut();
        });
        sup.auth.getSession().then(({data:{session}})=>{if(session)this._onSignedIn(session);});
    },
    toggleAuthModal(){
