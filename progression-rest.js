...
const ProgressionManager = (function(){
    ...
    return {
        ...
        // 🌟 login 完成时 AuthManager 调用（只运行一次设置 uid）
        onLoginDone:async function(uid, email){
            _cachedAuthUid=uid; data.email=email;
            var savedNick=data.nickname;
            console.log('🔑 onLoginDone: uid='+uid+' email='+email+' savedNick='+savedNick);
            var ok=await saveToCloud();
            console.log('  saveToCloud result: '+ok);
            var row=await loadFromCloud();
            console.log('  loadFromCloud row:', row?('nickname='+row.nickname):'null');
            if(row){loadFromRow(row);}
            if(savedNick){data.nickname=savedNick;console.log('  restored nickname: '+savedNick);}
            _saveCache();
            this.refreshUI();
            console.log('  final data.nickname='+data.nickname);
        },
        ...
        _forceSetAuth:function(uid,email){_cachedAuthUid=uid;data.email=email;},
        getAuthUid:function(){return _cachedAuthUid;},
...
var AuthManager = {
    ...
    init:function(){
        ...
        SupabaseREST.authGetUser().then(function(u){
            if(u) ProgressionManager.onLoginDone(u.id,u.email).then(function(){
    ...
    login:async function(){
        ...
        try{
            ...
            await ProgressionManager.onLoginDone(r.user.id, email);
    ...
    register:async function(){
        ...
        try{
            ...
            if(nickname&&(nickname.length<2||nickname.length>12)){err.textContent='昵称2-12字符';err.style.display='';btn.disabled=false;btn.textContent='注 册';return;}
            // ① 先注册
            ...
            // ② 注册成功，立刻设 uid + token → setNickname 才能用 getEffectiveId
            localStorage.setItem('supabase_auth_token',SupabaseREST.token);
            ProgressionManager._forceSetAuth(r.user.id, email);
            // ③ 设昵称
            if(nickname){var nr=await ProgressionManager.setNickname(nickname);if(!nr.ok){err.textContent=nr.error;err.style.display='';btn.disabled=false;btn.textContent='注 册';return;}}
            // ④ 同步云端
            await ProgressionManager.onLoginDone(r.user.id, email);
            ...
            if(typeof socket!=='undefined'&&socket&&socket.emit) socket.emit('register_auth',{token:SupabaseREST.token});
            toast2('✅ 注册成功',email+' · +50 Coin','#4caf50');
            btn.disabled=false;btn.textContent='注 册';
        }catch(e){err.textContent='网络错误';err.style.display='';btn.disabled=false;btn.textContent='注 册';}
    },
