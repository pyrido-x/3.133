...
var ProgressionManager = (function(){
    ...
    function applyRow(row){
        ...
        d.dailyCheckinDate=row.daily_checkin_date||''; d.checkinStreak=row.checkin_streak||0;
        d.dailyGameCoins=row.daily_game_coins||0; d.dailyGameCoinDate=row.daily_game_coin_date||'';
        d.email=row.email||''; d.nickname=row.nickname||sn;
        d.rankScore=row.rank_score!=null?row.rank_score:100; d.rankTier=row.rank_tier||'bronze';
        d.seasonGames=row.season_games||0; d.seasonWins=row.season_wins||0; d.seasonStart=row.season_start||'';
        if(row.stats)d.stats=row.stats;
        if(row.achievements)d.achievements=row.achievements;
        if(row.daily_challenges)d.dailyChallenges=row.daily_challenges;
    ...
    async function load(){
        ...
        if(authUid&&SupabaseREST.token){
            ...
            if(row){applyRow(row); console.log('☁️ 云加载 ok, nickname='+data.nickname); return;}
        }
        // 2. 缓存
        var c=cacheLoad();
    ...
    return {
        ...
        async onLogin(uid,email,nick,coinBonus){
            ...
            if(coinBonus)data.coins=coinBonus;
            cacheSave();
            if(SupabaseREST.token)await saveNow();
            refreshUI();
            console.log('✅ onLogin: uid='+uid+' nick='+(data.nickname||'')+' coins='+data.coins);
        },
        onLogout:function(){
        ...
        async onGameEnd(won){
            var self=ProgressionManager;
            data.gamesPlayed++;addXP(20);
            // coin
            {var t=_today();if(data.dailyGameCoinDate!==t){data.dailyGameCoins=0;data.dailyGameCoinDate=t;}if(data.dailyGameCoins<100){data.dailyGameCoins+=10;addCoins(10,'对局完成');}}
            if(self.isRankedGame)rankedGame(won);
            if(won){data._sess.won=true;data.gamesWon++;data.winStreak++;if(data.winStreak>data.bestWinStreak)data.bestWinStreak=data.winStreak;var b=80;if(data.winStreak>=3)b+=25;if(data.winStreak>=5)b+=50;if(data.winStreak>=10)b+=100;addXP(b);{var t2=_today();if(data.dailyGameCoinDate!==t2){data.dailyGameCoins=0;data.dailyGameCoinDate=t2;}data.dailyGameCoins+=15;addCoins(15,'胜利奖励');}_toast('🏆 胜利！','+'+(b+20)+'XP'+(data.winStreak>=3?' 🔸'+data.winStreak+'连胜！':''),'#4caf50');}
            else{data.winStreak=0;if(data._sess.turns>=5)addXP(10);}
            checkAch();checkDaily();
            await saveNow();
            refreshUI();
        },
