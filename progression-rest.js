...
const ProgressionManager = (function(){
    ...
    return {
        async init(){
            // 🌟 恢复登录态——AuthManager.init 之前执行
            var tok=localStorage.getItem('supabase_auth_token');
            if(tok){SupabaseREST.token=tok;}
            // 清除旧格式 ID
        ...
        onLoginDone:async function(uid, email){
            ...
            var savedNick=data.nickname;      // ← 记住本地昵称
            await saveToCloud();               // ← 推到云
            var row=await loadFromCloud();     // ← 拉回云
            if(row){loadFromRow(row);}
            if(savedNick) data.nickname=savedNick; // ← 本地昵称为准
            _saveCache();
            this.refreshUI();
            console.log('🔑 登录完成：'+email+' 昵称：'+(data.nickname||'(无)'));
        },
        ...
        getNickname:function(){
            ...
            // 已登录+没昵称 → 用邮箱前缀
            if(_cachedAuthUid&&data.email)return data.email.split('@')[0];
            // 未登录 → 游客
            return '游客';
        },
