...
const ProgressionManager = (function(){
    ...
    async function load(){
        ...
        if(row){console.log('☁️ 云端数据已加载');loadFromRow(row);}
        else{data=defaults();data._sess=defaults()._sess;_checkDaily();console.log('📦 游客模式（刷新清零）');}
    }
    ...
    return {
        ...
        onDeath:function(){data.stats.deaths++;data._sess.steppedOn=true;saveToCloud();},
