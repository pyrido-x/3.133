...
var SupabaseREST = {
    ...
    _loadTokens: function(){
        try{this.token=localStorage.getItem('supabase_auth_token')||null;
            this.refreshToken=localStorage.getItem('supabase_refresh_token')||null;}catch(e){}
    },
    _saveTokens: function(){
        try{if(this.token)localStorage.setItem('supabase_auth_token',this.token);
            else localStorage.removeItem('supabase_auth_token');
            if(this.refreshToken)localStorage.setItem('supabase_refresh_token',this.refreshToken);
            else localStorage.removeItem('supabase_refresh_token');}catch(e){}
    },
...
var ProgressionManager = (function(){
    ...
    function checkAch(){
        ...
        for(var key in checks){
            if(checks[key]){
                if(isGuestGame){
                    // 🌟 游客局：收集虚拟成就（对局结束展示，但不保存）
                    if(!data.achievements[key]){
                        if(!sess._guestAch)sess._guestAch=[];
                        if(sess._guestAch.indexOf(key)===-1)sess._guestAch.push(key);
                    }
                }else{
                    // 正常局：真实解锁
                    if(!data.achievements[key]){
                        ...
                        _toast(a.icon+' 成就解锁！',a.name+' — '+a.desc,'#ff9800');
                        addXP(30);
                    }
                }
            }
    ...
    return {
            ...
            if(tok){
                ...
                if(user){authUid=user.id;console.log('🔑 uid='+authUid);}
                    ...
                    if(await SupabaseREST.refreshAuth()){
                        ...
                        if(user){authUid=user.id;console.log('🔑 刷新后 uid='+authUid);}
                        else{console.log('⚠️ 刷新后仍无效，清除');SupabaseREST.logout();}
                    }else{console.log('⚠️ 无refresh_token，清除');SupabaseREST.logout();}
                }
            }
            // 清除旧格式 ID
        ...
        async onGameEnd(won){
            ...
            data.gamesPlayed++;
            var t=_today();
            var xpEarned=0, coinEarned=0, guestAchList=[];
            ...
            if(isGuestGame){
                ...
                // 收集虚拟成就
                guestAchList=(data._sess._guestAch||[]).slice();
                delete data._sess._guestAch;
            }else{
                // ══════════════════════════════════
                // ✅ 正常局：完整 XP + 成就 + Coin
                // ══════════════════════════════════
            ...
            refreshUI();
            // 🌟 对局结束总结弹窗（持久，点击 → 收回）
            _gameEndSummary(xpEarned,coinEarned,won,isGuestGame,guestAchList,self.hasGuest);
        },
...
// 🌟 对局结束总结弹窗（持久显示，点击 → 收回）
function _gameEndSummary(xp,coins,won,isGuest,guestAchList,hasGuest){
    ...
    // 虚拟成就
    if(guestAchList&&guestAchList.length>0){
        lines.push('<div style="font-size:11px;color:#999;margin-top:6px;border-top:1px solid #eee;padding-top:4px">🏅 本次达成（不计入）：</div>');
        var achDefs=ProgressionManager.getAchievements();
        for(var i=0;i<guestAchList.length;i++){
            var key=guestAchList[i];
            var ach=achDefs[key];
