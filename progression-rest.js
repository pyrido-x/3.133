...
function toast2(title,msg,color){
...
const ProgressionManager = (function(){
    ...
    function defaults(){return{
        ...
        _sess:{stomp:0,mine:0,blast:0,explored:0,turns:0,steppedOn:false,seenByEnemy:false,sp:0,minesPl:0,minesDf:0,blinks:0,treeJumps:0,blasts:0,firstTurn:true,usedSkill:false,stuckDeath:false,won:false}
    };}

    function saveToCloudNow(){return saveToCloud();}
    ...
    async function saveToCloud(){
        _saveCache();
        if(!SupabaseREST.ready()||!SupabaseREST.token){console.log('saveToCloud: skip (no ready/token)');return false;}
        _maybeStartSeason();
        var row={
        ...
        console.log('💾 saveToCloud: id='+row.id+' nickname='+row.nickname);
        var result=await SupabaseREST.dbUpsert('player_progress',row);
        console.log('  dbUpsert result: '+result);
        return result;
    }

    function loadFromRow(row){
        var def=defaults();
        var savedNick=data&&data.nickname ? data.nickname : ''; // 🌟 保留已有昵称
        if(row){
            data={xp:row.xp||0,level:row.level||1,gamesPlayed:row.games_played||0,gamesWon:row.games_won||0,
                ...
                nickname:row.nickname||savedNick, // 🌟 云有则用云，否则保留本地
    ...
    async function load(){
        ...
        data=defaults();data._sess=defaults()._sess;_checkDaily();console.log('📦 默认数据');
    }
    ...
    function _refreshUI(){
        ...
        var ab=document.getElementById("auth-btn");if(ab){var isLog=!!(SupabaseREST.token);if(isLog){var nn=data.nickname||(data.email?data.email.split("@")[0]:"");ab.innerText="👤 "+nn;ab.title=data.email||"";}else{ab.innerText="🔑 登录/注册";ab.title="";}}
    ...
    function checkAchievements(){
        ...
        var checks={
            ...
            grand_slam:data._sess.stomp>0&&data._sess.mine>0&&data._sess.blast>0,ghost:!data._sess.seenByEnemy&&(data._sess.stomp>0||data._sess.mine>0||data._sess.blast>0),survivor:data._sess.turns>=35,tianhu:data._sess.firstTurn&&data._sess.stomp>0,dihu:data._sess.firstTurn&&data._sess.steppedOn,omniscient:data._sess.explored>=data._sess._mapCells,caged:data._sess.stuckDeath,ascetic:data._sess.won&&!data._sess.usedSkill&&data._sess.turns>=15
        };
    ...
    return {
        async init(){
            // 🌟 ① 先恢复 token + uid（load 之前必须有 uid）
            var tok=localStorage.getItem('supabase_auth_token');
            if(tok){
                SupabaseREST.token=tok;
                var user=await SupabaseREST.authGetUser();
                if(user){_cachedAuthUid=user.id;if(!data)data=defaults();data.email=user.email||'';}
            }
            // 清除旧格式 ID
            var oldId=localStorage.getItem(PLAYER_ID_KEY);
            if(oldId&&oldId.indexOf('u_')===0){localStorage.removeItem(PLAYER_ID_KEY);}
            // ② 现在 load 能用自己的 uid 查到云端数据
            await load();
            this.refreshUI();
            // ③ 页面刷新时踢旧连接
        ...
        setNickname:async function(name){
            ...
            if(SupabaseREST.ready()&&SupabaseREST.token){
                ...
                console.log('🖊️ setNickname: name='+name+' pid='+pid+' token='+!!SupabaseREST.token);
                var claimed=await SupabaseREST.nicknameClaim(name,pid);
                if(!claimed){console.log('❌ nicknameClaim 失败');return{ok:false,error:'昵称已被占用了'};}
            }
            data.nickname=name;console.log('✅ data.nickname='+data.nickname);
            await saveToCloud();this.refreshUI();return{ok:true};
        },
        isRankedGame:false,
        ...
        onGameStart:function(N){data._sess=defaults()._sess;data._sess._mapCells=N||100;},
        ...
        onGameEnd:async function(won){
            data.gamesPlayed++;this.addXP(20);this.awardGameCoins();
            if(this.isRankedGame)_onRankedGame(won);
            if(won){data._sess.won=true;data.gamesWon++;data.winStreak++;if(data.winStreak>data.bestWinStreak)data.bestWinStreak=data.winStreak;var b=80;if(data.winStreak>=3)b+=25;if(data.winStreak>=5)b+=50;if(data.winStreak>=10)b+=100;this.addXP(b);this.awardWinCoins();toast('🏆 胜利！','+'+(b+20)+'XP'+(data.winStreak>=3?' 🔥'+data.winStreak+'连胜！':''),'#4caf50');}
            else{data.winStreak=0;if(data._sess.turns>=5)this.addXP(10);}
            checkAchievements();checkDaily();
            await saveToCloudNow(); // 🌟 等云写完再继续
            this.refreshUI();
        },
        onMinePlaced:function(){data.stats.minesPlaced++;data._sess.minesPl++;data._sess.usedSkill=true;checkAchievements();checkDaily();saveToCloud();},
        onMineDefused:function(){data.stats.minesDefused++;data._sess.minesDf++;data._sess.usedSkill=true;checkAchievements();checkDaily();saveToCloud();},
        onBlinkUsed:function(){data.stats.blinksUsed++;data._sess.blinks++;data._sess.usedSkill=true;checkAchievements();checkDaily();saveToCloud();},
        onTreeJumpUsed:function(){data.stats.treeJumpsUsed++;data._sess.treeJumps++;data._sess.usedSkill=true;checkAchievements();checkDaily();saveToCloud();},
        onBlastUsed:function(){data.stats.blastsUsed++;data._sess.blasts++;data._sess.usedSkill=true;checkAchievements();checkDaily();saveToCloud();},
        ...
        setAuthTemp:function(uid,email,nick,coins){_cachedAuthUid=uid;data.email=email;if(nick)data.nickname=nick;if(coins!==undefined)data.coins=coins;_saveCache();},
        _loadFromRaw:function(row){loadFromRow(row);_saveCache();},
        clearAuth:function(){_cachedAuthUid=null;data=defaults();_saveCache();this.refreshUI();},
    };
})();
...
var AuthManager = {
    ...
    init:function(){
        ...
        // 🌟 ProgressionManager.init() 已处理恢复，这里只做 UI 同步
        var tok=localStorage.getItem('supabase_auth_token');
        if(!tok)return;
        SupabaseREST.token=tok;
        SupabaseREST.authGetUser().then(function(u){
            if(u){
                ProgressionManager.setAuthTemp(u.id,u.email||'','',0);
                SupabaseREST.dbGet('player_progress',u.id).then(function(row){
                    if(row) ProgressionManager._loadFromRaw(row);
                    ProgressionManager.refreshUI();
                    if(typeof socket!=='undefined'&&socket&&socket.emit) socket.emit('register_auth',{token:tok});
                });
            }
            else AuthManager._onSignedOut();
        });
    },
    toggleAuthModal:function(){
        var modal=document.getElementById('auth-modal');
        if(modal.style.display==='flex'){modal.style.display='none';return;}
        if(!SupabaseREST.ready()){return;}
    ...
    register:function(){
        ...
        try{
            ...
            if(!nickname||nickname.length<2||nickname.length>12){err.textContent='昵称2-12字符，必须填写';err.style.display='';btn.disabled=false;btn.textContent='注 册';return;}
            // ① 注册账号
            ...
            // ② 有 token 了，设置 auth + 昵称
            localStorage.setItem('supabase_auth_token',SupabaseREST.token);
            ProgressionManager.setAuthTemp(r.user.id, email, nickname, 50);
            console.log('🖊️ 注册: '+nickname+' uid='+r.user.id);
            // ③ 先写昵称表
            if(SupabaseREST.token){
                var exists=await SupabaseREST.nicknameExists(nickname);
                if(exists){err.textContent='昵称 "'+nickname+'" 已被占用，请换一个';err.style.display='';btn.disabled=false;btn.textContent='注 册';return;}
                await SupabaseREST.nicknameClaim(nickname,r.user.id);
            }
            // ④ 写云端数据
            var row={
                id:r.user.id, xp:0, level:1, games_played:0, games_won:0, win_streak:0, best_win_streak:0,
                coins:50, daily_checkin_date:'', checkin_streak:0,
                daily_game_coins:0, daily_game_coin_date:'', email:email, nickname:nickname,
                rank_score:100, rank_tier:'bronze', season_games:0, season_wins:0, season_start:'',
                stats:{killsStomp:0,killsMine:0,killsBlast:0,deaths:0,minesPlaced:0,minesDefused:0,blinksUsed:0,treeJumpsUsed:0,blastsUsed:0,totalSPEarned:0,totalCellsExplored:0,mudHits:0},
                achievements:{}, daily_challenges:null, updated_at:new Date().toISOString()
            };
            var saved=await SupabaseREST.dbUpsert('player_progress',row);
            console.log('💾 注册保存: '+saved+' nickname='+nickname);
            // ⑤ 成功：关闭弹窗 + 刷新 UI + 通知
            document.getElementById('auth-modal').style.display='none';
            ProgressionManager.refreshUI();
            if(typeof socket!=='undefined'&&socket&&socket.emit) socket.emit('register_auth',{token:SupabaseREST.token});
            ...
            big.innerHTML='<div style="background:#fff;border-radius:16px;padding:30px 40px;text-align:center;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,0.4)"><div style="font-size:48px;margin-bottom:10px">🎉</div><div style="font-size:22px;font-weight:900;color:#4caf50;margin-bottom:8px">注册成功！</div><div style="font-size:15px;color:#555;margin-bottom:4px">'+email+'</div><div style="font-size:14px;color:#ff9800;font-weight:bold;margin-bottom:16px">昵称：'+nickname+' · +50 Coin</div><button onclick="this.parentElement.parentElement.remove()" style="padding:10px 30px;border:none;border-radius:8px;background:#4caf50;color:#fff;font-size:16px;font-weight:bold;cursor:pointer">开始游戏 🎮</button></div>';
            document.body.appendChild(big);
            // 5秒后自动消失
            setTimeout(function(){if(big.parentElement)big.remove();},5000);
            btn.disabled=false;btn.textContent='注 册';
        }catch(e){err.textContent='注册失败，请检查网络后重试';err.style.display='';btn.disabled=false;btn.textContent='注 册';console.error(e);}
        })();
    },
