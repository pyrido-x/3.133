...
var SupabaseREST = {
    ...
    rankedReplayInsert: async function(row){
        try{
            var r=await fetch(this.url+'/rest/v1/ranked_replays',{method:'POST',headers:Object.assign({},this._authHeaders(),{'Prefer':'return=minimal'}),body:JSON.stringify(row)});
            if(!r.ok)console.log('rankedReplayInsert failed',r.status,await r.text());
            return r.ok;
        }catch(e){console.log('rankedReplayInsert error',e.message);return false;}
    },
    ...
    function emptyData(){
        return {
            ...
            _sess:{stomp:0,mine:0,blast:0,explored:0,turns:0,steppedOn:false,seenByEnemy:false,
                   ...
                   usedSkill:false,stuckDeath:false,spComebackWindow:false,won:false,_mapCells:100}
        };
    }
