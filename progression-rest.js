...
function toggleLevelChart(){
    ...
    for(var i=0;i<LEVELS_ALL.length;i++){
        var lv=i+1,nxtLv=LEVELS_ALL[i+1];
        var need=nxtLv?nxtLv.xp-LEVELS_ALL[i].xp:'—';
        var isCur=lv===cur.level;
        h+='<tr style="'+(isCur?'background:#fff3e0;font-weight:bold':'')+'">';
        h+='<td style="padding:4px 10px">'+(isCur?'▶ ':'')+lv+'</td>';
        h+='<td style="padding:4px 10px">'+LEVELS_ALL[i].title+'</td>';
        h+='<td style="padding:4px 10px">'+LEVELS_ALL[i].xp+'</td>';
