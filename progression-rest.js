...
var ProgressionManager = (function(){
    ...
    return {
        ...
        async init(){
            ...
            try{authUid=localStorage.getItem('supabase_auth_uid')||null;}catch(e){}
            if(tok){
                var user=await SupabaseREST.getUser();
                if(user){authUid=user.id;try{localStorage.setItem('supabase_auth_uid',authUid);}catch(e){}console.log('🔑 uid='+authUid);}
                else{
                    console.log('⚠️ token 可能过期，尝试刷新...');
                    if(await SupabaseREST.refreshAuth()){
                        user=await SupabaseREST.getUser();
                        if(user){authUid=user.id;try{localStorage.setItem('supabase_auth_uid',authUid);}catch(e){}console.log('🔑 刷新后 uid='+authUid);}
        ...
        async onLogin(uid,email,nick,coinBonus){
            ...
            try{localStorage.setItem('supabase_auth_uid',uid);}catch(e){}
        ...
        onLogout:function(){
            ...
            try{localStorage.removeItem('supabase_auth_uid');}catch(e){}
            data=emptyData();data._sess=emptyData()._sess;
