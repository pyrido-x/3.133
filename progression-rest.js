...
const ProgressionManager = (function(){
    ...
    function defaults(){return{
        ...
        _sess:{stomp:0,mine:0,blast:0,explored:0,turns:0,steppedOn:false,sp:0,minesPl:0,minesDf:0,blinks:0,treeJumps:0,blasts:0,firstTurn:true}
    };}
    ...
    function _addXP(amount){
        ...
        data.level=Math.min(nl,LEVELS.length);saveToCloud();
    ...
    function checkAchievements(){
        ...
        var checks={
            ...
            grand_slam:data._sess.stomp>0&&data._sess.mine>0&&data._sess.blast>0,ghost:!data._sess.steppedOn&&data._sess.turns>=10,survivor:data._sess.turns>=20,tianhu:data._sess.firstTurn&&data._sess.stomp>0,dihu:data._sess.firstTurn&&data._sess.steppedOn
        };
...
//  等级图表弹窗
// ══════════════════════════════════════
function toggleLevelChart(){
    var m=document.getElementById('level-chart-modal');
    if(m.style.display==='flex'){m.style.display='none';return;}
    var cur=ProgressionManager.getLevelInfo();
    var all=ProgressionManager.getLevelInfo._all;
    var h='<div class="stats-section-title">📈 当前进度</div>';
    h+='<div style="text-align:center;margin:10px 0;font-size:18px;font-weight:900">Lv.'+cur.level+' <span style="color:#ff9800">'+cur.title+'</span></div>';
    h+='<div style="background:#e0e0e0;border-radius:8px;height:14px;overflow:hidden;margin:0 0 4px"><div style="width:'+cur.progress+'%;height:100%;background:linear-gradient(90deg,#4caf50,#8bc34a);border-radius:8px"></div></div>';
    h+='<div style="text-align:center;font-size:12px;color:#888;margin-bottom:16px">'+cur.xpIntoLevel+'/'+cur.xpNeeded+' XP → '+cur.nextTitle+'</div>';
    h+='<div class="stats-section-title">🗺️ 全部等级 (30级)</div>';
    h+='<div style="max-height:320px;overflow-y:auto;font-size:12px">';
    h+='<table style="width:100%;border-collapse:collapse;text-align:left">';
    h+='<tr style="background:#f5f5f5"><th style="padding:6px 10px">Lv</th><th style="padding:6px 10px">称号</th><th style="padding:6px 10px">累计 XP</th><th style="padding:6px 10px">升级需 XP</th></tr>';
    for(var i=0;i<LEVELS.length;i++){
        var lv=i+1,nxtLv=LEVELS[i+1];
        var need=nxtLv?nxtLv.xp-LEVELS[i].xp:'—';
        var isCur=lv===cur.level;
        h+='<tr style="'+(isCur?'background:#fff3e0;font-weight:bold':'')+'">';
        h+='<td style="padding:4px 10px">'+(isCur?'▶ ':'')+lv+'</td>';
        h+='<td style="padding:4px 10px">'+LEVELS[i].title+'</td>';
        h+='<td style="padding:4px 10px">'+LEVELS[i].xp+'</td>';
        h+='<td style="padding:4px 10px">'+need+'</td>';
        h+='</tr>';
    }
    h+='</table></div>';
    document.getElementById('level-chart-body').innerHTML=h;
    m.style.display='flex';
}
// 绑定 LEVELS 供等级图表使用
var LEVELS_ALL = null;
ProgressionManager.getLevelInfo._all=true;
