...
const SupabaseREST = {
    ...
    async authSignup(email, password){
        var r = await fetch(this.url+'/auth/v1/signup', {
            method:'POST', headers:this._headers(),
            body:JSON.stringify({email:email,password:password})
        });
        var data = await r.json();
        if (r.ok && data.access_token) { this.token = data.access_token; return {ok:true, user:data.user}; }
        return {ok:false, error:data.msg||data.message||'注册失败'};
    },

    async authLogin(email, password){
        var r = await fetch(this.url+'/auth/v1/token?grant_type=password', {
            method:'POST', headers:this._headers(),
            body:JSON.stringify({email:email,password:password})
        });
        var data = await r.json();
...
const ProgressionManager = (function(){
    var STORAGE_KEY='digitalfog_progression_v2', PLAYER_ID_KEY='digitalfog_player_id';
    var _cachedAuthUid=null, _syncTimer=null, data=null;
    var LEVELS=[
    ...
    var ACHIEVEMENTS={
        ...
        grand_slam:{name:'大满贯',desc:'一局内用三种方式击杀',icon:'🎯'},ghost:{name:'幽灵',desc:'一局未被敌人踩到所在格',icon:'👻'},
    ...
    var RANK_LABELS={bronze:'🟤 青铜',silver:'⚪ 白银',gold:'🟡 黄金',platinum:'🟣 铂金',diamond:'💎 钻石'};

    function getPlayerId(){
        var p=localStorage.getItem(PLAYER_ID_KEY);
        if(!p){p='u_'+crypto.randomUUID();localStorage.setItem(PLAYER_ID_KEY,p);}
        return p;
    }
    ...
    function defaults(){return{
        ...
        rankScore:1000,rankTier:'bronze',seasonGames:0,seasonWins:0,seasonStart:'',
    ...
    function saveLocal(){
        try{var o={};for(var k in data){if(k!=='_sess')o[k]=data[k];}localStorage.setItem(STORAGE_KEY,JSON.stringify(o));}catch(e){}
    }
    function saveAll(){saveLocal();markCloudDirty();}

    function markCloudDirty(){
        clearTimeout(_syncTimer);_syncTimer=setTimeout(saveToCloud,2000);
    }
    function saveToCloudNow(){clearTimeout(_syncTimer);saveToCloud();}

    async function loadFromCloud(){
        if(!SupabaseREST.ready())return null;
        var eid=getEffectiveId();
        var row=await SupabaseREST.dbGet('player_progress',eid);
        if(row)return row;
        if(eid!==getPlayerId()){return await SupabaseREST.dbGet('player_progress',getPlayerId());}
        return null;
    }
    ...
    function loadFromRow(row){
        ...
        if(row){
            data={xp:row.xp||0,level:row.level||1,gamesPlayed:row.games_played||0,gamesWon:row.games_won||0,
                ...
                rankScore:row.rank_score||1000,rankTier:row.rank_tier||'bronze',seasonGames:row.season_games||0,
    ...
    function loadFromLocal(){
        try{var r=localStorage.getItem(STORAGE_KEY);if(r){data=JSON.parse(r);var d=defaults();for(var k in d){if(!(k in data)&&k!=='_sess')data[k]=d[k];}}else{data=defaults();}}catch(e){data=defaults();}
        data._sess=defaults()._sess;_checkDaily();
    }

    async function load(){
        var row=await loadFromCloud();
        if(row){console.log('☁️ REST 云端数据已加载');loadFromRow(row);}
        else{loadFromLocal();console.log('📦 本地数据');}
        saveLocal();
    }
    ...
    function _maybeStartSeason(){
        ...
        if(!data.seasonStart||data.seasonStart!==key){data.seasonStart=key;data.rankScore=1000;data.rankTier='bronze';data.seasonGames=0;data.seasonWins=0;}
    }
    function _computeRankTier(s){if(s>=2000)return'diamond';if(s>=1700)return'platinum';if(s>=1400)return'gold';if(s>=1200)return'silver';return'bronze';}
    ...
    function _addXP(amount){
        ...
        for(var i=LEVELS.length-1;i>=0;i--){if(data.xp>=LEVELS[i].xp){nl=i+1;break;}}
        data.level=Math.min(nl,LEVELS.length);saveAll();
        if(data.level>oldLv){for(var lv=oldLv+1;lv<=data.level;lv++)toast('🎉 等级提升！','Lv.'+lv+' '+LEVELS[lv-1].title,'#4caf50');}
        _refreshUI();
    }
    function _addCoins(amount,reason){data.coins+=amount;saveAll();if(amount>0)toast('🪙 +'+amount+' Coin',reason||'','#ff9800');_refreshUI();}
    ...
    function _refreshUI(){
        ...
        var rk=document.getElementById('rank-display');if(rk)rk.innerHTML=(RANK_LABELS[data.rankTier]||'🟤 青铜')+' <span style="font-size:9px;opacity:0.7">'+data.rankScore+'分</span>';
        var rbm=document.getElementById('rank-badge-mini');if(rbm)rbm.textContent=RANK_LABELS[data.rankTier]||'🟤 青铜';
    ...
    function _onRankedGame(won){_maybeStartSeason();data.seasonGames++;if(won){data.seasonWins++;data.rankScore+=25;}else{data.rankScore=Math.max(0,data.rankScore-15);}_recalcRank();}
    ...
    return {
        async init(){
            ...
            await load();
            if(SupabaseREST.ready())saveToCloudNow();
            this.refreshUI();
        },
        ...
        getNickname:function(){return data.nickname||'';},
        setNickname:async function(name){
            ...
            data.nickname=name;saveAll();this.refreshUI();return{ok:true};
        },
        isRankedGame:false,
        ...
        onKill:function(type){data.stats['kills'+type.charAt(0).toUpperCase()+type.slice(1)]++;data._sess[type]++;checkAchievements();checkDaily();saveAll();},
        onDeath:function(){data.stats.deaths++;data._sess.steppedOn=true;saveAll();},
        ...
        onMinePlaced:function(){data.stats.minesPlaced++;data._sess.minesPl++;checkAchievements();checkDaily();saveAll();},
        onMineDefused:function(){data.stats.minesDefused++;data._sess.minesDf++;checkAchievements();checkDaily();saveAll();},
        onBlinkUsed:function(){data.stats.blinksUsed++;data._sess.blinks++;checkAchievements();checkDaily();saveAll();},
        onTreeJumpUsed:function(){data.stats.treeJumpsUsed++;data._sess.treeJumps++;checkAchievements();checkDaily();saveAll();},
        onBlastUsed:function(){data.stats.blastsUsed++;data._sess.blasts++;checkAchievements();checkDaily();saveAll();},
        onSPEarned:function(n){data.stats.totalSPEarned+=n;data._sess.sp+=n;checkAchievements();checkDaily();saveAll();},
        onCellExplored:function(){data.stats.totalCellsExplored++;data._sess.explored++;checkDaily();saveAll();},
        onTurnEnd:function(){data._sess.turns++;if(data._sess.turns>=20)checkAchievements();},
        onMudHit:function(){data.stats.mudHits++;saveAll();},
        claimDaily:function(id){if(!data.dailyChallenges)return;var ch=data.dailyChallenges.challenges.find(function(c){return c.id===id;});if(!ch||!ch.completed||ch.claimed)return;ch.claimed=true;this.addXP(50+Math.floor(Math.random()*30));saveAll();this.refreshUI();},
        ...
        clearAuth:function(){_cachedAuthUid=null;loadFromLocal();this.refreshUI();}
    };
})();
...
var AuthManager = {
    ...
    login:async function(){
        ...
        document.getElementById('auth-modal').style.display='none';this._onSignedIn(r.user);
    },
    register:async function(){
        var nickname=(document.getElementById('auth-nickname')||{}).value;
        ...
        if(nickname&&(nickname.length<2||nickname.length>12)){err.textContent='昵称2-12个字符';err.style.display='';return;}
        if(!email||!password){err.textContent='请填写邮箱和密码';err.style.display='';return;}
        ...
        if(email.indexOf('@')<0||email.indexOf('.')<0){err.textContent='请输入有效的邮箱地址';err.style.display='';return;}
        err.style.display='none';
        if(nickname){
            var nr=await ProgressionManager.setNickname(nickname);
            if(!nr.ok){err.textContent='昵称: '+nr.error;err.style.display='';return;}
        }
        var r=await SupabaseREST.authSignup(email,password);
        if(!r.ok){err.textContent=r.error;err.style.display='';return;}
        localStorage.setItem('supabase_auth_token',SupabaseREST.token);
        document.getElementById('auth-modal').style.display='none';ProgressionManager.addCoins(50,'注册奖励');this._onSignedIn(r.user);
    },
