...
var SupabaseREST = {
    ...
    _readJsonSafe: async function(r){
        var text='';
        try{text=await r.text();}catch(e){}
        if(!text)return{};
        try{return JSON.parse(text);}catch(e){return{message:text};}
    },
    _friendlyError: function(err,fallback){
        var raw='';
        if(err)raw=err.error_description||err.msg||err.message||String(err);
        var s=String(raw||'').toLowerCase();
        if(s.indexOf('paused')>=0||s.indexOf('project')>=0&&s.indexOf('resume')>=0||s.indexOf('failed to fetch')>=0||s.indexOf('network')>=0){
            return 'Supabase 项目已暂停或暂时不可用，请先在 Supabase 控制台点击 Resume project';
        }
        return raw||fallback;
    },
    ...
    signup: async function(e,p,nick){
        try{
            ...
            var d=await this._readJsonSafe(r);
            if(r.ok&&d.access_token){this._storeAuth(d);return{ok:true,user:d.user};}
            return{ok:false,error:this._friendlyError(d,'Signup failed')};
        }catch(err){return{ok:false,error:this._friendlyError(err,'Signup failed')};}
    },
    login: async function(e,p){
        try{
            ...
            var d=await this._readJsonSafe(r);
            if(r.ok&&d.access_token){this._storeAuth(d);return{ok:true,user:d.user};}
            return{ok:false,error:this._friendlyError(d,'Login failed')};
        }catch(err){return{ok:false,error:this._friendlyError(err,'Login failed')};}
    },
    ...
    rankedReplayList: async function(mode, authUid, matchType){
        try{
            matchType=matchType||'ranked';
            var base=this.url+'/rest/v1/ranked_replays?select=id,created_at,room_id,match_type,player1_id,player2_id,player1_name,player2_name,player1_rank,player2_rank,winner_id,winner_name&order=created_at.desc&limit=100';
            if(matchType==='casual'){
                base+='&match_type=eq.casual';
                if(mode==='mine'&&authUid)base+='&or=(player1_id.eq.'+encodeURIComponent(authUid)+',player2_id.eq.'+encodeURIComponent(authUid)+')';
            }else if(mode==='mine'&&authUid){
                base+='&and=(or(match_type.eq.ranked,match_type.is.null),or(player1_id.eq.'+encodeURIComponent(authUid)+',player2_id.eq.'+encodeURIComponent(authUid)+'))';
            }else{
                base+='&or=(match_type.eq.ranked,match_type.is.null)';
            }
...
var ProgressionManager = (function(){
    ...
    var SHOP_SKINS = [
        ...
        {id:'blast_spark',slot:'blast',name:'火花轰击',rarity:'common',price:80,desc:'轰击时炸开短促火花和碎片。'},
        {id:'blast_arc',slot:'blast',name:'电弧轰击',rarity:'rare',price:180,desc:'蓝紫闪电沿轰击路径弹跳，并在每格中心激发电波。'},
        {id:'blink_trail',slot:'blink',name:'残影闪',rarity:'common',price:80,desc:'闪现时拖出细碎残影。'},
        {id:'blink_leaf',slot:'blink',name:'落叶树闪',rarity:'rare',price:180,desc:'起落点旋出叶片圆环，路径扩散绿色正弦波。'},
        {id:'mine_pulse',slot:'mine',name:'脉冲埋雷',rarity:'common',price:80,desc:'埋雷时压下一圈脉冲。'},
        {id:'mine_shadow',slot:'mine',name:'暗影埋雷',rarity:'rare',price:180,desc:'埋雷时散开暗色碎影。'},
        {id:'defuse_scan',slot:'defuse',name:'扫描拆解',rarity:'common',price:80,desc:'拆雷时出现扫描框。'},
        {id:'defuse_silver',slot:'defuse',name:'银线拆解',rarity:'rare',price:180,desc:'银色曲线从玩家格伸向目标，扫描后数字发光上浮。'}
    ];
    var SHOP_WHISPERS = [
    ...
    var SLOT_LABELS = {cell:'人物格子纹理',blink:'闪/树闪特效',blast:'轰击特效',mine:'埋雷特效',defuse:'拆除特效',kill:'击杀特效'};
    ...
    function defaultShopData(){
        return {ownedSkins:[],equippedSkins:{cell:null,blink:null,blast:null,mine:null,defuse:null,kill:null},ownedWhispers:[]};
    }
    function normalizeShop(shop){
        ...
        ['cell','blink','blast','mine','defuse','kill'].forEach(function(slot){
    ...
    return {
        ...
        onDeath:function(cause){data.stats.deaths++;if(cause==='stomp')data._sess.steppedOn=true;checkAch();checkDaily();doSave();},
        ...
        onTurnEnd:function(actionType){data._sess.turns++;if(actionType!=='spawn')data._sess.firstTurn=false;if(data._sess.turns>=35)checkAch();},
...
var AuthManager = {
    ...
    register:async function(){
        ...
        try{
            ...
            if(/\s/.test(nick)){err.textContent='昵称不能包含空格或换行';err.style.display='';btn.disabled=false;btn.textContent='注 册';return;}
            if(nick==='游客'){err.textContent='昵称不能使用“游客”';err.style.display='';btn.disabled=false;btn.textContent='注 册';return;}
...
var Shop={
    ...
    renderPreview:function(s){
        var id=this.esc(s.id), cls='shop-preview preview-'+id;
        if(s.id==='cell_circuit')return'<div class="'+cls+' preview-epic" data-label="电路核心"><div class="preview-atlas circuit"></div></div>';
        if(s.id==='cell_ink')return'<div class="'+cls+' preview-epic" data-label="水墨涌动"><div class="preview-atlas ink"></div></div>';
        if(s.id==='kill_crown')return'<div class="'+cls+' preview-epic" data-label="冠冕爆发"><div class="preview-atlas crown"></div></div>';
        if(s.rarity==='epic'&&s.slot!=='cell')return'<div class="'+cls+'"><div class="preview-epic-placeholder">史诗素材占位</div></div>';
        if(s.slot==='cell')return'<div class="'+cls+'"><div class="shop-preview-cell p1-bg '+this.esc(s.cssClass||'')+'">7</div></div>';
        if(s.slot==='blink'){
            if(s.id==='blink_leaf')return'<div class="'+cls+'"><span class="dot a" style="background:#58c744"></span><span class="dot b" style="background:#7ed35a"></span><svg viewBox="0 0 160 82"><path class="glow" d="M34 41 C55 17, 72 66, 95 41 S125 25, 140 41"/><path class="core" d="M34 41 C55 17, 72 66, 95 41 S125 25, 140 41"/></svg><span class="preview-leaf" style="--angle:0deg;left:78px;top:33px"></span><span class="preview-leaf" style="--angle:90deg;left:78px;top:33px"></span><span class="preview-leaf" style="--angle:180deg;left:78px;top:33px"></span><span class="preview-leaf" style="--angle:270deg;left:78px;top:33px"></span></div>';
            return'<div class="'+cls+'"><span class="dot a" style="background:#5aaee8"></span><span class="dot b" style="background:#8bd3ff"></span><svg viewBox="0 0 160 82"><path d="M34 41 C62 38, 86 38, 126 41"/></svg></div>';
        }
        if(s.slot==='blast'){
            if(s.id==='blast_arc')return'<div class="'+cls+'"><span class="dot a" style="background:#6de7ff"></span><span class="dot b" style="background:#8b6cff"></span><svg viewBox="0 0 160 82"><defs><linearGradient id="preview-arc-grad" x1="25" y1="0" x2="140" y2="0" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="#6de7ff"/><stop offset="52%" stop-color="#8a74ff"/><stop offset="100%" stop-color="#b35cff"/></linearGradient></defs><path class="main" d="M32 42 L48 31 L62 50 L79 29 L96 48 L113 32 L129 41"/><path class="core" d="M34 40 L50 35 L63 47 L81 33 L98 45 L116 36 L128 40"/></svg></div>';
            return'<div class="'+cls+'"><span class="dot a" style="background:#ffb34d"></span><span class="dot b" style="background:#ff8a3d"></span><svg viewBox="0 0 160 82"><path d="M34 41 L64 35 L92 47 L126 41"/></svg></div>';
        }
        if(s.slot==='mine')return'<div class="'+cls+'"><div class="preview-ring"></div><div class="preview-kill-mark" style="position:absolute">'+(s.id==='mine_shadow'?'伏':'埋')+'</div></div>';
        if(s.slot==='defuse')return'<div class="'+cls+'"><div class="preview-scan"></div>'+(s.id==='defuse_silver'?'<div class="preview-number">314</div>':'')+'</div>';
        if(s.slot==='kill')return'<div class="'+cls+'"><div class="preview-kill-mark">'+this.esc(s.mark||'击破')+'</div></div>';
        return'<div class="'+cls+'"><div class="preview-epic-placeholder">预览</div></div>';
    },
    render:function(){
    ...
    renderStore:function(){
        ...
        var html='<div class="shop-box"><div style="font-weight:900;font-size:18px">神秘盲盒</div><div class="shop-meta">你可以从这里获得任何东西……？</div><button onclick="Shop.openBox()">开启 '+ProgressionManager.getBoxCost()+' Coin</button><div class="shop-result" id="shop-box-result"></div></div>';
        var groups=[['cell','纹理'],['blink','闪'],['blast','轰'],['mine','埋雷'],['defuse','拆'],['kill','击杀']];
        groups.forEach(function(group){
            var groupSkins=skins.filter(function(s){return s.slot===group[0];});
            if(!groupSkins.length)return;
            html+='<div style="font-weight:900;margin:14px 0 8px">'+self.esc(group[1])+'</div><div class="shop-grid">';
            groupSkins.forEach(function(s){
                var owned=state.ownedSkins.indexOf(s.id)>=0,eq=state.equippedSkins[s.slot]===s.id;
                html+='<div class="shop-card">'+self.renderPreview(s)+'<div class="shop-card-name">'+self.esc(s.name)+'</div>'
