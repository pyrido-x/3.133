...
const ProgressionManager = (function(){
    ...
    return {
        async init(){
            // 🌟 先恢复 token + 查出 uid（AuthManager.init 之前）
            var tok=localStorage.getItem('supabase_auth_token');
            if(tok&&!SupabaseREST.token){
                SupabaseREST.token=tok;
                var user=await SupabaseREST.authGetUser();
                if(user){_cachedAuthUid=user.id;console.log('🔑 已恢复登录: '+user.email);}
            }
        ...
        onAuthLogin:async function(uid,email){
            _cachedAuthUid=uid;data.email=email;
            var savedNick=data.nickname;
            await saveToCloud(); // 等云端写入完成
            var row=await loadFromCloud();
            if(row){loadFromRow(row);if(savedNick)data.nickname=savedNick;_saveCache();}
            this.refreshUI();
        },
        setAuth:function(uid,email){_cachedAuthUid=uid;data.email=email;saveToCloudNow();this.refreshUI();},
        _setRaw:function(uid,email){_cachedAuthUid=uid;data.email=email;},
...
var AuthManager = {
    ...
    register:async function(){
        ...
        try{
            ...
            // 先设置 auth 上下文（uid + token）
            ProgressionManager._setRaw(r.user.id, email);
            localStorage.setItem('supabase_auth_token',SupabaseREST.token);
            // 再设昵称（此时 token 和 uid 都就绪）
            if(nickname){
            ...
            // 最后完整保存到云 + 刷新 UI
            await ProgressionManager.onAuthLogin(r.user.id, email);
            ProgressionManager.addCoins(50,'注册奖励');
            document.getElementById('auth-modal').style.display='none';
            this._onSignedIn(r.user); // 会触发 saveToCloudNow
            var t=document.getElementById('toast-container');
