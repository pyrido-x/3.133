...
io.on('connection', (socket) => {
    ...
    socket.on('join_matchmaking', (data) => {
        ...
        if (matchmakingQueue.length >= 2) {
            ...
            // 分配基础数据层（使用昵称）
            room.players[0] = { id: p1Id, authUid: s1 && s1._authUid || null, roleId: 1, color: PLAYER_CONFIGS[1].color, name: (s1&&s1._nickname) || PLAYER_CONFIGS[1].name, alive: true, votedRemap: false, votedRerp: false };
            room.players[1] = { id: p2Id, authUid: s2 && s2._authUid || null, roleId: 2, color: PLAYER_CONFIGS[2].color, name: (s2&&s2._nickname) || PLAYER_CONFIGS[2].name, alive: true, votedRemap: false, votedRerp: false };
