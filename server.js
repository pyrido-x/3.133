...
let globalOnlineCount = 0; // 全服唯一在线身份数量
const presenceCounts = new Map(); // presenceId -> socket count

const MAP_SIZE_DEFAULT = 10;

function broadcastOnlineCount() {
    globalOnlineCount = presenceCounts.size;
    io.emit('server_online_count', { count: globalOnlineCount });
}

function setSocketPresence(socket, presenceId) {
    if (!presenceId || socket._presenceId === presenceId) return;
    if (socket._presenceId) {
        const oldCount = (presenceCounts.get(socket._presenceId) || 1) - 1;
        if (oldCount > 0) presenceCounts.set(socket._presenceId, oldCount);
        else presenceCounts.delete(socket._presenceId);
    }
    presenceCounts.set(presenceId, (presenceCounts.get(presenceId) || 0) + 1);
    socket._presenceId = presenceId;
    broadcastOnlineCount();
}

function removeSocketPresence(socket) {
    if (!socket._presenceId) return;
    const oldCount = (presenceCounts.get(socket._presenceId) || 1) - 1;
    if (oldCount > 0) presenceCounts.set(socket._presenceId, oldCount);
    else presenceCounts.delete(socket._presenceId);
    socket._presenceId = null;
    broadcastOnlineCount();
}

function getPresenceId(socket) {
    if (socket._authUid) return 'u:' + socket._authUid;
    if (socket._clientId) return 'c:' + socket._clientId;
    return 's:' + socket.id;
}
...
// 创建一个干净的房间对象
function createRoomObject(slots, mapSize = MAP_SIZE_DEFAULT, terrainWeights = null, allowSpectate = true) {
    ...
    return {
        ...
        allowSpectate: allowSpectate  // 🌟 是否允许观战（默认 true）
    };
}
...
function processVote(room, roomId, type) {
    let allVoted = true;
    let humanCount = 0;
    const voteProp = type === 'ranked_rematch' ? 'votedRematch' : (type === 'remap' ? 'votedRemap' : 'votedRerp');
    ...
    // 2. 同步灯光状态给前端
    let voteStatus = room.players.map(p => p ? p[voteProp] : false);
    ...
    if (humanCount > 0 && allVoted) {
        // 清理所有人（包括 AI）的投票状态
        room.players.forEach(p => {
            if (p) { p.votedRemap = false; p.votedRerp = false; p.votedRematch = false; }
        });
        io.to(roomId).emit('clear_votes');

        if (type === 'ranked_rematch') {
            if (!room.isRanked || !room.lastGameState || room.lastGameState.gameActive !== false) return;
            ...
            io.to(roomId).emit('ranked_rematch_approved');
...
// ══════════════════════════════════════
//  登录身份映射：token → 当前最近连接。登录本身不踢连接，入座/匹配时按 authUid 去重。
// ══════════════════════════════════════
...
io.on('connection', (socket) => {
    setSocketPresence(socket, getPresenceId(socket));

    socket.on('register_presence', (data) => {
        const clientId = (data && data.clientId) || '';
        if (clientId) socket._clientId = clientId;
        setSocketPresence(socket, getPresenceId(socket));
    });

    // 🌟 注册认证 token（只记录身份，不因多标签互踢）
    socket.on('register_auth', (data) => {
        ...
        if (token) {
            ...
            socket._authUid = authUid;
            if (authUid && socket.roomId && rooms[socket.roomId]) {
                const roomPlayer = rooms[socket.roomId].players.find(p => p && p.id === socket.id && !p.isAI);
                if (roomPlayer) roomPlayer.authUid = authUid;
            }
            setSocketPresence(socket, getPresenceId(socket));
        } else {
            // 清除旧 token 关联
    ...
    socket.emit('server_online_count', { count: globalOnlineCount });
    
    // ---- 功能 1：匹配模式 (双人对战快速开局) ----
    socket.on('join_matchmaking', (data) => {
        // 🌟 存储昵称
        if (data && data.nickname) socket._nickname = data.nickname;
        if (data && data.authUid) socket._authUid = data.authUid;
        socket._rankLabel = (data && data.rankLabel) || '';
        socket._rankScore = (data && data.rankScore) || 0;

        if (socket._authUid) {
            const duplicateQueued = matchmakingQueue.some(id => {
                const s = io.sockets.sockets.get(id);
                return s && s.id !== socket.id && s._authUid === socket._authUid;
            });
            if (duplicateQueued) {
                socket.emit('match_error', '该账号已经在匹配队列中，请先取消另一个页面的匹配。');
                return;
            }
        }
        ...
        if (matchmakingQueue.length >= 2) {
            ...
            let p1Socket = io.sockets.sockets.get(p1Id);
            let p2Index = matchmakingQueue.findIndex(id => {
                const s = io.sockets.sockets.get(id);
                return s && (!p1Socket || !p1Socket._authUid || !s._authUid || p1Socket._authUid !== s._authUid);
            });
            if (p2Index === -1) {
            ...
            let p2Id = matchmakingQueue.splice(p2Index, 1)[0];

            let roomId = 'MATCH_' + Math.random().toString(36).substr(2, 6).toUpperCase();
            let room = createRoomObject([1, 1, 0, 0], 10, null, false); // 快速匹配不允许观战
            room.isRanked = !!(data && data.ranked);
            rooms[roomId] = room;
            ...
            // 分配基础数据层（使用昵称）
            room.players[0] = { id: p1Id, authUid: s1 && s1._authUid || null, roleId: 1, color: PLAYER_CONFIGS[1].color, name: (s1&&s1._nickname) || PLAYER_CONFIGS[1].name, rankLabel: (s1&&s1._rankLabel)||'', rankScore: (s1&&s1._rankScore)||0, alive: true, votedRemap: false, votedRerp: false };
            room.players[1] = { id: p2Id, authUid: s2 && s2._authUid || null, roleId: 2, color: PLAYER_CONFIGS[2].color, name: (s2&&s2._nickname) || PLAYER_CONFIGS[2].name, rankLabel: (s2&&s2._rankLabel)||'', rankScore: (s2&&s2._rankScore)||0, alive: true, votedRemap: false, votedRerp: false };
            ...
            const configs = {
                1: { name: room.players[0].name, color: room.players[0].color, authUid: room.players[0].authUid, rankLabel: room.players[0].rankLabel, rankScore: room.players[0].rankScore },
                2: { name: room.players[1].name, color: room.players[1].color, authUid: room.players[1].authUid, rankLabel: room.players[1].rankLabel, rankScore: room.players[1].rankScore }
            };

            const isRanked = (data && data.ranked) ? true : false;
            const p1Nick = (s1&&s1._nickname)||''; const p2Nick = (s2&&s2._nickname)||'';
            const hasGuest = !p1Nick||p1Nick==='游客'||!p2Nick||p2Nick==='游客';
            io.to(p1Id).emit('game_start', { roomId, myRoleId: 1, maxPlayers: 2, slotTypes: room.slotTypes, activeRoleIds: room.activeRoleIds, configs, gridData: room.gridData, isRanked, hasGuest });
            io.to(p2Id).emit('game_start', { roomId, myRoleId: 2, maxPlayers: 2, slotTypes: room.slotTypes, activeRoleIds: room.activeRoleIds, configs, gridData: room.gridData, isRanked, hasGuest });
    ...
    socket.on('create_room', (data) => {
        ...
        let allowSpectate = (data.allowSpectate !== undefined) ? data.allowSpectate : true;
        let room = createRoomObject(slots, mapSize, terrainWeights, allowSpectate);
    ...
    socket.on('join_room', (data) => {
        ...
        const authUid = socket._authUid || null;
        if (authUid && room.players.some(p => p && !p.isAI && p.authUid === authUid && p.id !== socket.id)) {
            socket.emit('join_error', '该账号已经在这个房间中，不能重复占用玩家席位。');
            return;
        }
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (emptySlotIndex === -1) {
            // 🌟 如果房间不允许观战，拒绝加入
            if (room.allowSpectate === false) {
                socket.emit('join_error', '房间已满（不允许观战）');
                return;
            }
            // 🌟 如果房间满人且游戏已经开始，则让新加入的人自动成为旁观者
            if (room.gameStarted) {
    ...
    socket.on('cast_vote', (data) => {
        ...
        if (player) {
            let type = data.type;
            let voteProp = type === 'ranked_rematch' ? 'votedRematch' : (type === 'remap' ? 'votedRemap' : 'votedRerp');
            player[voteProp] = !player[voteProp];
            processVote(room, roomId, type);
        }
        // 如果投票者没有实体身份，但它是该全 AI 房间的创建者/代跑宿主，则单方面强制重置
        else if ((room.aiHostId === socket.id || roomId === '000000') && (!room.slotTypes || !room.slotTypes.some(s => s === 1))) {
            executeInstantReset(room, roomId, data.type);
        }
    });

    socket.on('ai_host_reset', (data) => {
        const roomId = socket.roomId || data.roomId;
        if (!roomId || !rooms[roomId]) return;
        const room = rooms[roomId];
        const hasNoHumanSlots = !room.slotTypes || !room.slotTypes.some(s => s === 1);
        if (!hasNoHumanSlots) return;
        if (room.aiHostId !== socket.id && roomId !== '000000') return;
        executeInstantReset(room, roomId, data.type);
    });
