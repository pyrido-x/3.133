...
io.on('connection', (socket) => {
    ...
    socket.on('sync_state', (data) => {
        if (data && data.roomId && rooms[data.roomId]) {
            ...
            // 🌟 新增：测试房间自动重开逻辑
            if (data.roomId === '000000' && data.gameState && !data.gameState.gameActive && !room.restarting) {
                ...
                setTimeout(() => {
                    ...
                    // 广播给所有人重开地图通知
                    io.to('000000').emit('map_regenerated', { gridData: room.gridData });
                }, 30000); // 30000 毫秒 = 30 秒
            }
        }
    });
    ...
    socket.on('cast_vote', (data) => {
        ...
        // 🌟【核心修复点 2】：移除了对 `player.alive` 的判定。
        // 允许死亡、但未断线的玩家在游戏结束后正常按下投票按钮。
        if (player) {
            let type = data.type; 
            let voteProp = type === 'remap' ? 'votedRemap' : 'votedRerp';
        ...
        // 🌟 新增拦截：如果投票者没有实体身份，但它是该全 AI 房间的创建者/代跑宿主，则单方面强制重置
