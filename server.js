...
// 创建一个干净的房间对象
function createRoomObject(slots, mapSize = MAP_SIZE_DEFAULT) {
...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', (data) => {
        ...
        let room = createRoomObject(slots, mapSize);
        ...
        if (aiCount === nonEmptyCount && nonEmptyCount > 0) {
            ...
            // 兜底防错
            const totalPlayers = room.players.filter(p => p !== null).length;
            if(safeCells.length < totalPlayers) safeCells = [{r:0,c:0}, {r:0,c:1}, {r:1,c:0}, {r:1,c:1}];
...
function executeInstantReset(room, roomId, type) {
    ...
    if (type === 'remap') {
        ...
        if (!room.slotTypes || !room.slotTypes.some(s => s === 1)) {
