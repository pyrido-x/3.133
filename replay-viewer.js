...
function buildReplayHTML(data) {
    ...
    H.push('E("tl").textContent="回合 "+(logIdx+1)+"/"+totalTurns;');
    ...
    // Button state
    H.push('var btns=document.querySelectorAll(".ctrl button");if(btns[0])btns[0].disabled=(FI<=0);if(btns[1])btns[1].disabled=(FI>=S.length-1);');
    H.push('}');
