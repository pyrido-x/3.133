// replay-viewer.js — buildReplayHTML(data) 自包含回放 HTML
...
function buildReplayHTML(data) {
    ...
    H.push('<select id="vs" onchange="R()"></select>');
    H.push('<label><input type="checkbox" id="sn" onchange="R()">编号</label>');
    H.push('<label><input type="checkbox" id="sp" onchange="R()">路径</label>');
    ...
    H.push('<script>');
    H.push('var D='+json+';');
    H.push('var CL='+JSON.stringify(colors)+';');
    H.push('var S=D.snaps,G=D.grid,LL=D.log,C=D.cfg,Z=D.names,N=C.mapSize,F=0;');
    H.push('var AC=C.activeRoleIds||[],TR={white:"tw",gray:"tg",blue:"tb",green:"te"};');
    H.push('function E(id){return document.getElementById(id);}');
    H.push('function H(s){return String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");}');
    H.push('function NXT(){if(F<S.length-1){F++;R();}}');
    H.push('function PRV(){if(F>0){F--;R();}}');
    ...
    // render
    H.push('function R(){');
    H.push('var f=S[Math.min(F,S.length-1)];if(!f)return;');
    H.push('var flog=f.log||[],isInit=(F===0),isEnd=F>=S.length-1;');
    H.push('var totalRows=LL.length||0;');
    H.push('if(isInit){E("tl").textContent="初始状态 / 共"+totalRows+"回合";E("cl").textContent="";}');
    H.push('else{');
    H.push('  var curRow=flog.length;');
    H.push('  E("tl").textContent="回合 "+curRow+"/"+totalRows;');
    H.push('  E("cl").textContent="当前: "+(Z[f.cp]||"P"+f.cp);}');

    H.push('var vs=E("vs"),pe=vs?vs.value:"omni",om=pe=="omni";');
    H.push('var sN=E("sn").checked,sP=E("sp").checked,cs=N>20?Math.max(20,Math.floor(480/N)):50;');
    H.push('var gEl=E("g");gEl.style.gridTemplateColumns="repeat("+N+","+cs+"px)";');

    // fog
    H.push('var pk={};');
    H.push('if(!om&&f.players[pe]&&f.players[pe].knowledge){var ka=f.players[pe].knowledge;for(var ki=0;ki<ka.length;ki++)pk[ka[ki]]=true;}');
    ...
    H.push('if(kn&&f.mines){for(var mi=0;mi<f.mines.length;mi++){var mk=f.mines[mi],ci=mk.indexOf(":");if(ci<1)continue;var mc=mk.substring(ci+1).split(",");if(parseInt(mc[0])===r&&parseInt(mc[1])===c){ir+="<div class=\\"mo\\"></div>";break;}}}');
    H.push('for(var pid in f.players){var p=f.players[pid];if(!p||p.r!==r||p.c!==c)continue;var pn=parseInt(pid);if(p.alive){ir+="<div class=\\"po p"+pn+"\\">"+nm+"</div>";}else{ir+="<div class=\\"do\\">☠</div>";}}');
    ...
    H.push('var sh="";for(var pid in pp){var clr=CL[pid]||"#888",pt=pp[pid];if(!pt||pt.length<2)continue;var d="M"+(pt[0][1]*cs+cs/2)+","+(pt[0][0]*cs+cs/2);for(var j=1;j<pt.length;j++)d+=" L"+(pt[j][1]*cs+cs/2)+","+(pt[j][0]*cs+cs/2);sh+="<polyline points=\\""+d+"\\" fill=\\"none\\" stroke=\\""+clr+"\\" stroke-width=\\"2\\" stroke-dasharray=\\"6,3\\" opacity=\\"0.8\\"/>";}');
    H.push('svg.innerHTML=sh;');

    // status
    H.push('var td=E("td");if(pd)td.innerHTML="<span style=\\"color:#f44336\\">已阵亡-全知视角</span>";else if(isInit)td.innerHTML="<span style=\\"color:#ff9800\\">初始状态</span>";else if(om)td.innerHTML="<span style=\\"color:#333\\">全知视角</span>";else td.innerHTML="<span style=\\"color:#4caf50\\">"+H(Z[pe]||"P"+pe)+" 视角</span>";');

    // log — show accumulated rows from this snapshot, last row's cp column highlighted
    H.push('var th=E("lth"),hdr="<tr>";');
    H.push('for(var i=0;i<AC.length;i++)hdr+="<th style=\\"color:"+(CL[AC[i]]||"#333")+"\\">"+H(Z[AC[i]]||"P"+AC[i])+"</th>";');
    H.push('th.innerHTML=hdr+"</tr>";');
    H.push('var tb=E("ltb"),htm="";');
    H.push('if(isInit||!flog||flog.length===0){htm="<tr><td colspan="+AC.length+">等待对局开始</td></tr>";}');
    H.push('else{');
    H.push('var cpIdx=AC.indexOf(f.cp);');
    H.push('for(var t=0;t<flog.length;t++){');
    H.push('var isLast=(t===flog.length-1);');
    H.push('htm+="<tr>";for(var j=0;j<AC.length;j++){htm+="<td"+(isLast&&j===cpIdx?" class=\\"cur\\"":"")+">"+H(String(flog[t][j]||""))+"</td>";}');
    H.push('htm+="</tr>";}}');
    H.push('tb.innerHTML=htm;setTimeout(function(){var lc=E("lc");if(lc)lc.scrollTop=lc.scrollHeight;},50);');

    // buttons
    H.push('E("btn-prev").disabled=(F<=0);E("btn-next").disabled=(F>=S.length-1);');
    H.push('}');

    // init
    H.push('E("vs").innerHTML="<option value=\\"omni\\">全知视角</option>";');
    H.push('for(var pid in C.playerConfigs){if(C.playerConfigs[pid]&&!C.playerConfigs[pid].isEmpty)E("vs").innerHTML+="<option value=\\""+pid+"\\">"+H(Z[pid]||"P"+pid)+"</option>";}');
    H.push('F=0;R();');
