...
function buildReplayHTML(data) {
    ...
    H.push('var flog=f.log||[],logIdx=flog.length>0?flog.length-1:0;');
    H.push('if(FI===0){E("tl").textContent="初始状态 / 共"+(L.length||0)+"回合";E("cl").textContent="";}');
    H.push('else{');
    H.push('var tn=flog?flog.length:0;');
    H.push('E("tl").textContent="回合 "+tn+"/"+(L.length||0);');
    ...
    H.push('if(FI===0||!flog||flog.length===0){htm="<tr><td colspan="+AC.length+">等待对局开始</td></tr>";}');
    H.push('else{');
    H.push('var cpCol=AC.indexOf(f.cp);');
    H.push('for(var t=0;t<flog.length;t++){');
    H.push('var isLastRow=(t===flog.length-1);');
    H.push('htm+="<tr>";');
    H.push('for(var j=0;j<AC.length;j++){');
    H.push('var isCur=(isLastRow&&j===cpCol);');
    H.push('htm+="<td"+(isCur?" class=\\"cur\\"":"")+">"+EH(String(flog[t][j]||""))+"</td>";}');
    H.push('htm+="</tr>";}}');
